...
atlasdir=/media/ageinglab/Expansion/Atlas/SIGMA_Brain/sigma_wistar_rat_brain_templatesandatlases_version1-2-1_2022-08-26/SIGMA_Wistar_Rat_Brain_TemplatesAndAtlases_Version1.2.1/SIGMA_Rat_Anatomical_Imaging/SIGMA_Rat_Anatomical_InVivo_Template;
###/misc/geminis/navalj/prueba/atlas/TohokuUniv/correctOrientation;###
###Directorio en Don Clusterio### 
atlasbrain=${atlasdir}/SIGMA_InVivo_Brain_Template_Masked_125mm_reorient.nii.gz;
atlasbrain5mm=${atlasdir}/SIGMA_InVivo_Brain_Template_Masked_5mm_reorient.nii.gz;
atlasbrain5mmMask=${atlasdir}/SIGMA_InVivo_Brain_Template_Masked_5mm_reorient_mask.nii.gz;
...
### BET Estructural
ls ${sourceDir}/derivatives/ppBOLD/${sub}/anat/sub-*_T2w.nii.gz | cut -d . -f 1 | parallel N4BiasFieldCorrection -i {}.nii.gz -o {}_p.nii.gz -t [0.3,0.01,100];
...
for i in $(ls -d ${sourceDir}/derivatives/ppBOLD/${sub});do echo $i;
	...
	${FSLDIR}/bin/fslmaths $j -Tmean -thr $liminf2 -bin -mas ${atlasdir}/SIGMA_InVivo_CSFWM_5mm_mask.nii.gz ${j}_CSFWMmask_5mm.nii.gz
